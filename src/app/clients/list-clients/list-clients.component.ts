...
export class ListClientsComponent implements OnInit, OnDestroy {
  ...
  delete(client: ClientModelTable) {
    this.httpSubscriptions.push(this.httpService
      .delete(client.id)
      .subscribe(_ =>this.snackBarManager
      .show(`O Cliente ${client.name} foi deletado com sucesso!`)))


  }
  update(client: ClientModelTable) {
    this.router.navigate(['clients/edit-client:id',client.id])
  }
}
